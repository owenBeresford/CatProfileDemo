...
import { SET_CATS, SET_A_CAT, SET_KEY, CatStateType, CatAction, KnowACat } from '../types/CatState';
...
export class CatState implements KnowACat {
    ...
    //public updateCats:storeCats;
    //public current:Cat;
    //public setCat:storeACat;
    store:Store;
    public key:string;
    ...
    public constructor(  ) {
        ...
        const SELF=this;
        const Zero={ allCats:[] as Array<Cat>, cat:defaultCat(null, 0) as Cat, key:"0aList"  };
        ...
        API.getAll(undefined).then((dd) => {
            ...
            SELF.store.dispatch({ type:SET_CATS, collection: importList.data as Array<Cat>});
            // SELF.updateCats(importList.data);
        });
    ...
    public currentCats():Array<Cat> {
        ...
        console.log("currentCats() state ", new Date(), ttt);
        return ttt.allCats ?? [];
    }

    public current():Cat {
        const ttt= this.store.getState();
        console.log("current() state ", new Date(), ttt);
        return ttt.cat;
    }

    isCatGood(a:Cat):void {
        if (a.ID === null) {
            throw new Error("Cats must have an ID code, how did it gets it's label off?");
        }
    ...
    //    this.setCat(a);
        ...
        console.log("Replacing cat " + a.ID, ttt[a.ID], a);
        ttt[a.ID] = a;
    ...
    //  this.updateCats(tmp);
        if (a.ID === this.current().ID) {
            this.store.dispatch({type:SET_A_CAT, single:defaultCat(null, tmp.length) });
        //    this.setCat( defaultCat(null, this.currentCats.length));
        }
    }
    ...
    public changeCat(dom: HTMLElement ):void { 
      ...
      //  let dom = e!.currentTarget as HTMLElement;
        ...
        //  this.updateCat(this.currentCats[parseInt(id, 10)]);
        }
    }
