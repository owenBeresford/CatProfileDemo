...
const ShowCat: React.FC<ShowCatProps> = (props: ShowCatProps) => {
  ...
  const age: string =
    new Date().getUTCFullYear() - cat.dob.getUTCFullYear() + " years old";
  return (
    <div className="cat popup">
      <dl>
        ...
        <dd>
          ...
          <p className="inset"> {cat.name} </p>
        </dd>
        <dt>Expressed Gender </dt>
        <dd>{cat.gender}</dd>
        <dt>Date of birth </dt>
        <dd>
          <time dateTime={cat.dob.toString()}>
            {renderDate(cat.dob)}/ {age}
          </time>
        </dd>
        <dt>About me </dt>
        <dd>{cat.about}</dd>
