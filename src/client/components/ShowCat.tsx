...
const ShowCat: React.FC<ShowCatProps> = (props: ShowCatProps) => {
  ...
  if(!props.current || parseInt(ID || "", 10)!==props.current.ID) {
    if(!ID || (props.current && !props.current.ID)) {
      ...
      return <div className="error popup">Data loading.. no ID and no data; pls talk to a dev.</div>;
    }
  }  
  ...
  function deleteCat(e: React.MouseEvent<HTMLAnchorElement, MouseEvent>):void {
    if(!props.current) { return; } // this button should be disabled if no cat
  ...
  const age: string =
    new Date().getUTCFullYear() - props.current.dob.getUTCFullYear() + " years old";
  return (
      ...
      <dl>
        ...
        <dt>
          ...
          {props.isChild ? (
            ...
            <span className="goBack">
              <NavLink to={"/"} onClick={deleteCat} > &#9249; </NavLink>
            </span>
            </>
          )}
          <span className="goBack">
