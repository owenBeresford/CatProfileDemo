...
const SignupCat:React.FC<SignupProps> =function(props: SignupProps):React.ReactElement<SignupProps> {
  const { ID } = useParams();
  ...
  return (
    <div className="signupContainer ">
      <>
        ...
        {spread(signupScreen, props.current, props.updateCat, props.removeCat, setSignupScreen)}
      </>
    </div>
  );
}
...
function spread(
  ...
  switch (signupScreen) {
    ...
    case 2:
      return (<CatScreen2 build={buildCat} incTab={push} updateCat={setBuildingCat} removeCat={removeCat} />);
    default:
